...
(гейт CI «BLS only on the paid push_root» в каждом workflow).
...
- У всех трёх — тест конфига `MIN_GROSS[game] ≥` индексного `MIN_GROSS` ($0.20).
...
Утверждение «такого теста нет ни на одном шейпе» ложно: `header_convention_holds` есть в **каждой
живой форме**, юнит-тестом в `src/lib.rs` — он проверяет дискриминатор `sha256("account:Escrow")[..8]`
и офсеты `donor` 8..40 / `salt` 40..72, то есть ровно те байты, которые читает
`crown-indexer::recognize::decode_birth`. Юнитом, а не litesvm — чтобы не зависеть от SBF-сборки и
не быть тихо пропущенным. Третий шейп, `payout-table`, вместе со своим тестом удалён на `P7.6`
(форма без потребителя).
