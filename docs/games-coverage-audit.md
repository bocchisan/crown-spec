...
## 0. Бейзлайн (что прогоняется прямо сейчас)

| Игра | logic-unit | canister-unit | pocket-ic | e2e | ончейн-форма |
|---|---|---|---|---|---|
| auction | 20 ✅ | 26 ✅ | 5 ✅ (`endpoint_e2e.rs`) | пусто | `two-outcome` ✅ (litesvm) |
| conditional-funding | 19 ✅ | 18 ✅ | 5 ✅ (`endpoint_e2e.rs`) | пусто | `two-outcome` ✅ |
| conditional-tasks | 19 ✅ | 29 ✅ | 3+5+1 ✅ (`birth_e2e` + `endpoint_e2e` + `schnorr_probe`) | пусто | `two-outcome` ✅ |
| subscription | — (нет `logic/`) | — (нет канистры) | — | **S1 ✅ devnet** (create/release/cancel/refund) | **`stream` 16 ✅ litesvm + 4 ✅ salt** |
| common (общий крейт) | 22 ✅ | | | | |

Все suite'ы зелёные (свежая сборка против current common): auction 20+26+5(pocket-ic),
funding 19+18+5(pocket-ic), tasks 19+29+3+5+1(pocket-ic), common 22, stream 16(litesvm)+4(salt).
`schnorr_probe` доказывает: `bootstrap` под `key_1` → резолвер (подписной путь канистр работает; §7).

**Слой эндпоинтов (`canister/src/lib.rs`):** частично покрыт pocket-ic харнессом (`endpoint_e2e.rs` в
каждой канистре, +прокси-фикстура `e2e/relay-proxy` — реле с циклами, т.к. ingress их не несёт).
Закрыты инварианты оплаты `request_signature` (`Underpaid` — неоплаченный не делает работы; «нет вердикта»
без списания — `NotDecided`/`NotFound`; `WrongTarget`), отбой мусорного голоса в `inspect_message`, queries.
**Ещё 🔵 (форж пруфов/devnet):** пруф рождения `register_entry`, пруф веса `vote`, threshold-подпись
`Signed`, деньги, `bootstrap`/`init`.

**Незакрытый этап у каждой игры** — «E2e через реальную сеть» (A5 / F5 / T5 / S1), затем прод. Директории
`e2e/` у всех пусты.
...
Ленивое создание **без пруфа → `Derived` echo, ноль записи** `[редк., головной инвариант]` ❌ ·
...
порядок отказов регистрации (Disabled→Floor→ProfileMin→Reputation→Duration→Deadline) ✅ · репутация гейтится
только при `min_reputation>0`, отсутствие пруфа = ниже, на-барьере проходит `[редк.]` ✅ · границы duration
включительны + дедлайн-граница + TimeOverflow ✅ · идемпотентная материализация + text_hash ✅ · только
получатель, чужой→NotFound ✅ · verdict() None до Decided ✅ · ленивый Tick финализирует ✅ · счётчик профиля
строго растёт, равный/ниже→StaleCounter `[редк.]` ✅ · профиль-кап, но существующие получатели ещё обновляются
`[редк.]` ✅ `profiles_are_capped_but_existing_recipients_still_update` · `add_vote` дедуп+порог+**V_MAX** ✅ · `task_id`
...
`set_profile` (`ProfileMinBelowFloor`/`NotRecipient`/`StaleCounter`-обвязка) · `vote` `inspect_message` ·
...
## 6. Расхождения спека↔код — РАЗРЕШЕНЫ переработкой аукциона
...
**Слой 3 — e2e через devnet.** Инфраструктура Solana-периметра **поднята**: сплиттер `EiLH5hCH…` +
two-outcome фабрика `9Cjb4…` задеплоены на devnet, донор `crown-index-e2e-donor` фондирован (SOL + тест-USDC
`4zMMC9sr…`), индекс — на локальном dfx.
...
- **`A5`/`F5`/`T5` (канистерные игры) — подписной путь РАЗБЛОКИРОВАН, доказан.** Фундамент поднят
  (локальная dfx-реплика, деплой индекса+tasks). Прежний «блокер» оказался **несовпадением имени ключа**:
  конфиг задаёт `threshold_key = "dfx_test_key"`, но dfx 0.32 schnorr не поддерживает вовсе («unsupported
  management canister method»), а pocket-ic-server 13 провижнит Ed25519/Bip340 Schnorr под именем **`key_1`**
  (проверено пробником по mgmt-канистре). Собрав tasks с профилем `pocketic` (`threshold_key = "key_1"`),
  `bootstrap` → **KeyBootstrapped** и `get_resolver` → резолвер (тест `canister/tests/schnorr_probe.rs`,
  зелёный). **Подписная цепочка работает.**
  - ⚠️ **Конфиг-баг для деплоя:** `dfx_test_key` не заведётся ни на реальном IC, ни в PocketIC. Для боевого
    деплоя `threshold_key` должен быть `key_1` (IC mainnet) или `test_key_1` (IC testnet-сабнет), не
    `dfx_test_key` (локально-dfx-only). Касается всех трёх канистр (`config/*.toml`).
