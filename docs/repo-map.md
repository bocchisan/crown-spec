...
└─ crown-games/
   ...
   ├─ conditional-funding/
   ├─ auction/
   ├─ subscription/       без канистры: форма `stream` + docs/ + офчейн-e2e
   └─ direct-settlement/  без канистры и своей программы: форма транзакции + logic/ (арифметика)
                          + любые будущие игры
```
...
`conditional-funding`, `auction`) и по отдельному job'у на каждую бесканистерную — `subscription` и
`direct-settlement`. У обеих проверяется именно **отсутствие** канистры/резолвера: у первой это всё,
что можно проверить, у второй к этому добавляются fmt/clippy/test её `logic/` и то, что он zero-dep. Пер-игровых `<game>/.github/` больше нет; они существовали и не исполнялись.
...
Исключений из этой раскладки два, и оба — игры без канистры:

- `subscription`: детерминированная рассрочка на форме `stream`, **без канистры/логики/резолвера** —
  только `docs/` + `e2e/` (вся ончейн-логика в форме `stream`, `crown-factory`).
- `direct-settlement`: прямой донат с комиссией — **без канистры, резолвера и своей ончейн-программы**:
  `docs/` + zero-dep `logic/` (разбиение суммы, флоры, правило подающего). Своей ончейн-части нет
  вовсе — игра собирает транзакцию поверх `spl_token` и `crown-splitter`, поэтому и `config/` у неё
  нет: бейкать не во что.
